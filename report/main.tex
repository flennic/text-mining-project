...
\definecolor{custom_five}{HTML}{edc951}
...
\begin{figure}[hbt]
	\center
	\includegraphics[width=1.0\textwidth]{img/kingmanwomanqueen}
...
\begin{itemize}
    \item Shows achieved accuracy's on train and test
    \item Also show learning curves
    \item Maybe a word on training times
\end{itemize}

All four model combinations and their achieved accuracies can be seen in figure \ref{tab:results}. The accuracies for training have dropout enabled. Figure \ref{fig:ffn_w2v_results} shows the accuracies for the FFN with Word2Vec embeddings, figure \ref{fig:ffn_bert_results} shows the accuracies for the FFN with DistilBERT, figure \ref{fig:lstm_w2v_results} shows the accuracies for the LSTM with Word2Vec embeddings and figure \ref{fig:lstm_bert_results} shows the accuracies for the LSTm with DestilBERT embeddings. 
...
\begin{figure}[hbt]
	...
	\begin{tikzpicture}
          ...
          \addplot[color=custom_four] coordinates {(2.3,ffnw2v)
                                (11.65,ffnbert)
                                (6.35,lstmw2v)
                                (12.1,lstmbert)};
          \end{axis}
...
\begin{itemize}
    \item Say the obvious for this problem set
    \item Refer to BERTs classification Token
    \item Limitations: There are a lot!
    \item Compare to related work, also important (do research)
\end{itemize}
...
\begin{itemize}
    \item BERT helps, for the high price of computational power, even if left static
    \item DistilBERT is quite good if the last percentages do no matter that much
    \item DeepLearning becomes even more resource hungry
    \item GPT2
\end{itemize}
